...
class GlossaryTabMixin:
                                ...
                                for line in f:
                                    if ":" in line:
                                        ...
                                        if val.startswith('"') and val.endswith('"'):
                                            val = val[1:-1]
                                        for t in re.findall(r'[a-zA-Z]+', val.lower()):
                                            if len(t) >= 3:
                                                src_freq[t] = src_freq.get(t, 0) + 1
                sorted_entries = sorted(src_freq.items(), key=lambda x: -x[1])
    ...
    def _mod_translate(self):
        ...
        if not entries:
            ...
            return
        model = self.ollama_model.get()
        tgt_display = self._m_tgt_var.get()
        if not model or model == "(none)":
        ...
        def _run():
            try:
                ...
                self.after(0, lambda: self._m_progress.set(0))
                result = self.engine._call_ollama(model,
                    f"Translate each English word to {tgt_display}. Return exactly 'word: translation' per line.\n" +
                ...
                checked = {}
                for src, freq in entries:
                    if src in translated:
                        checked[src] = translated[src]
                self._m_checked = checked
                self._m_page = 0
                self.after(0, lambda: self._mod_update_display())
                self.after(0, lambda: self.log(f"[MOD] Translated {len(checked)}/{len(entries)} terms"))
            except Exception as e:
                tb = traceback.format_exc()
    ...
    def _mod_update_display(self, filter_text=""):
        ...
        for idx, (src_tok, freq) in enumerate(page):
            row = idx
            ctk.CTkLabel(self._m_result_frame, text=src_tok, anchor="w").grid(row=row, column=0, sticky="w", padx=5)
            e = ctk.CTkEntry(self._m_result_frame, width=300)
            e.grid(row=row, column=1, sticky="ew", padx=5)
            e.insert(0, checked.get(src_tok, ""))
            ctk.CTkLabel(self._m_result_frame, text=str(freq), anchor="e", width=40).grid(row=row, column=2, sticky="e", padx=5)
    ...
    def _mod_load(self):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                parts = line.strip().split(":", 1)
                if len(parts) == 2:
                    entries.append((parts[0].strip(), parts[1].strip(), 0))
        if entries:
